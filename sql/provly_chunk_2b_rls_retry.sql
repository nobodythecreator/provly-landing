-- ============================================================================
-- PROVLY MIGRATION - CHUNK 2b: RLS + POLICIES (RETRY)
-- The cross-org tables and indexes from Chunk 2 already succeeded.
-- This contains only the RLS enable + public.org_id() + all policies.
...
-- Pattern: SELECT/INSERT/UPDATE/DELETE where org_id = public.org_id()
...
BEGIN
  ...
  LOOP
    EXECUTE format('CREATE POLICY "%s_select" ON %I FOR SELECT USING (org_id = public.org_id())', t, t);
    EXECUTE format('CREATE POLICY "%s_insert" ON %I FOR INSERT WITH CHECK (org_id = public.org_id())', t, t);
    EXECUTE format('CREATE POLICY "%s_update" ON %I FOR UPDATE USING (org_id = public.org_id())', t, t);
    EXECUTE format('CREATE POLICY "%s_delete" ON %I FOR DELETE USING (org_id = public.org_id())', t, t);
  END LOOP;
END $$;
...
CREATE POLICY "partnerships_select" ON org_partnerships FOR SELECT
  USING (requesting_org_id = public.org_id() OR receiving_org_id = public.org_id());
CREATE POLICY "partnerships_insert" ON org_partnerships FOR INSERT
  WITH CHECK (requesting_org_id = public.org_id());
