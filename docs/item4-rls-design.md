...
**Design v1.3 · Sep 16 2026 · approved Sep 9; v1.1 (Sep 10) corrected the role inventory and one table name found while building PR (a); v1.2 records the five PR (b) decisions B1–B5; v1.3 records the four PR (c) decisions C1–C4 (all Sep 16).** Decisions R1–R7 + drift D1–D3 + B1–B5 + C1–C4 locked.
...
**C1–C4 (v1.3, locked Sep 16) — what "signed" means, and who corrects what.**
- **C1 Record lock — service notes.** `approved` is a seal, not a flag: a BEFORE trigger freezes every content field for every tier; the only change an approved note accepts is an explicit reopen (approved → submitted) by operate/manage, written to `audit_log` (`note_reopened` — `audit_log.action` is VARCHAR(20)). Approval stamps `approved_by = my_staff_id()`, `approved_at = now()` server-side; deliver can never set approved and never moves a note to another author or client. Approved notes are undeletable by anyone.
- **C2 Same lock, three documents.** Incidents lock at `reviewed_at` (review layer — status, review_notes — stays editable by operate/manage; reopen clears the review, `incident_reopened`); quarterly summaries lock at `submitted_to_sc_at` / status submitted (`summary_reopened`). Deliver files and edits its own unreviewed incident, never reviews. No reviewed incident or submitted summary is deletable.
- **C3 Client identity is manage-only.** operate may UPDATE `persons` freely except name, date of birth, Medicaid #, ID #, admission/discharge dates and `is_active` — a trigger refuses those changes below manage. Intake (INSERT) is manage and not compliance_director.
- **C4 EVV corrections are a supervisor's.** A deliver login INSERTs its own session on a visible person and, while it is open, may set only `clock_out_at` / `clock_out_lat` / `clock_out_lng`; once clocked out the row is frozen for deliver in every column (r1: `geofence_valid` and `exceptions` included — evidence, not the worker's to touch); every correction requires operate/manage, with the v20.0.10 reason + edit log (`evv_edit_log` INSERT is office-tier). Sessions are never deleted.
- Also enforced by trigger: deliver changes only `status` on its own shift; below manage, a message update may only set `read_at`. Every trigger passes service-role / SQL-editor writes (no `auth.uid()`) untouched.
- Write policy shape: one `<t>_insert_tier` / `_update_tier` / `_delete_tier` per guarded table (116 policies); `audit_log` and `evv_edit_log` append-only; `organizations` UPDATE = owner/admin; `org_members`, `invites`, `subscription_events` take no app writes. Context membership (day-to-day roster) is operate; contexts themselves are manage.
...
| **(c)** write | v20.0.13 | Write policies §5 (116 tier policies, `provly_v13_writes`); C1–C2 record locks with audited reopen; C3 identity guard; C4 EVV deliver guard; shift status-only and message read-receipt triggers; append-only audit/edit logs. | Front-line invite gate **off** — app dropdown/Invite-to-app and `send-invite` `ALLOWED_INVITE_ROLES` (r1: the edge function had its own allowlist) — operators and DSPs invitable; coworker names via `staff_directory_v` (14 embeds aliased `staff:`, 8 name pickers; the two hinted incident embeds stay on `staff`); Reopen on approved notes and reviewed incidents; EVV correction control office-tier only; refused writes surface the database's own message. |
